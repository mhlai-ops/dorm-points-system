...
import { ArrowLeft, ArrowRight, Camera, Check, ChevronDown, ClipboardList, Eye, EyeOff, Gift, History, LogIn, QrCode, RefreshCw, ScanLine, ShieldCheck, Sparkles, UserRound, X } from "lucide-react";
...
const parseAuth = (raw: string | null): AuthState | null => {
  try {
    ...
    const auth = parsed as Partial<AuthState>;
    return auth.version === 1 && typeof auth.token === "string" && isCompactJws(auth.token) && typeof auth.signedInAt === "string" ? auth as AuthState : null;
  } catch { return null; }
};
const readAuth = (): AuthState | null => {
  try {
    return parseAuth(window.localStorage.getItem(AUTH_KEY)) || parseAuth(window.sessionStorage.getItem(AUTH_KEY));
  } catch { return null; }
};
const saveAuth = (remember: boolean, token?: string) => {
  try {
    const state: AuthState = { token: token || createAuthToken(), signedInAt: new Date().toISOString(), version: 1 };
    const serialized = JSON.stringify(state);
    const primary = remember ? window.localStorage : window.sessionStorage;
    const secondary = remember ? window.sessionStorage : window.localStorage;
    secondary.removeItem(AUTH_KEY);
    primary.setItem(AUTH_KEY, serialized);
    return true;
  } catch { return false; }
};
const clearAuth = () => {
...
const isLog = (value: unknown): value is Log => {
  if (!value || typeof value !== "object") return false;
  const item = value as Partial<Log>;
  return typeof item.id === "string" && typeof item.studentId === "string" && typeof item.at === "string" && typeof item.item === "string" && Number.isFinite(item.delta) && Number.isFinite(item.balance);
};
const load = <T,>(key: string, fallback: T, validate?: (value: unknown) => value is T): T => {
  try {
...
const persist = (students: Student[], logs: Log[]) => {
  try {
    ...
    // 儲存空間被封鎖或已滿時，繼續使用記憶體狀態，避免整個 App 白屏。
  }
};
...
const SUN_ICON = "/morning-joy-sun.jpeg";
...
function Login({ onLogin, busy }: { onLogin: (account: string, password: string, remember: boolean) => void; busy: boolean }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!normalizedAccount || !password) {
      toast.error("請輸入帳戶號碼及帳戶密碼。");
      return;
    }
    onLogin(normalizedAccount, password, remember);
  };
  return <div className="login-page"><div className="login-aside"><div className="entry-brand"><Mark/><div><strong>晨樂加油站</strong><small>晨樂 · 積分加油站</small></div></div><div className="chapter-label">01 / ACCESS</div><p className="kicker">MORNING JOY / 2026</p><h1>把每天的好表現，<em>記下來。</em></h1><p className="lede">為老師、家長和小朋友設計的積分加油站。記下每天的好表現，讓每一次努力都看得見。</p><div className="aside-note"><Sparkles size={17}/><span>今天，也一起讓宿舍變得更好。</span></div><div className="workflow-strip"><div><QrCode size={16}/><strong>掃描</strong><small>找宿生</small></div><div><span className="workflow-number">+5</span><strong>記分</strong><small>即時更新</small></div><div><History size={16}/><strong>追蹤</strong><small>可撤銷</small></div></div></div><div className="login-card"><div className="paper-holes"><i/><i/><i/></div><div className="card-kicker">管理者登入 / DUTY LOG</div><h2>進入你的<br/><span>值日工作台</span></h2><p className="muted">請使用已授權的帳號及密碼登入。</p><form onSubmit={submit}><label>帳號<input value={account} onChange={e => setAccount(e.target.value)} placeholder="輸入帳戶號碼" autoComplete="username"/></label><label>密碼<div className="password-input-wrap"><input value={password} onChange={e => setPassword(e.target.value)} type={showPassword ? "text" : "password"} placeholder="輸入帳戶密碼" autoComplete="current-password"/><button className="password-visibility-button" type="button" onClick={() => setShowPassword(value => !value)} aria-label={showPassword ? "隱藏密碼" : "顯示密碼"} aria-pressed={showPassword} title={showPassword ? "隱藏密碼" : "顯示密碼"}>{showPassword ? <EyeOff size={20}/> : <Eye size={20}/>}</button></div></label><label className="remember-row"><input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)}/><span>保持登入狀態</span><small>{remember ? "下次開啟可直接登入" : "關閉瀏覽器後需要重新登入"}</small></label><button className="primary wide" type="submit" disabled={busy}><LogIn size={18}/> {busy ? "連線中…" : "登入工作台"} {!busy && <ArrowRight size={17}/>}</button></form><div className="login-footer"><span>資料僅保存在此裝置</span><span className="dot"/> <span>HTTPS 可安全使用相機</span></div></div></div>;
}
...
export default function Home() {
  ...
  const login = async (account: string, password: string, remember: boolean) => {
    ...
    try {
      ...
      toast.error(message.includes("帳戶號碼或帳戶密碼不正確") ? "帳戶號碼或帳戶密碼不正確，請再試一次。" : "無法連接同步服務，請稍後再試。");
    }
  };
  const logout = () => {
    clearAuth();
